...
export function WorkspaceSwitcher({ value, onChange }: { value: WorkspaceId; onChange: (value: WorkspaceId) => void }) {
  ...
  return <div className="workspace-switcher">
    ...
    {open && <div className="workspace-menu">{WORKSPACES.map((workspace) => <button key={workspace.id} disabled={!workspace.enabled} onClick={() => { if (workspace.enabled) { onChange(workspace.id); setOpen(false); } }}>
