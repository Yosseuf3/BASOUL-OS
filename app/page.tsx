...
function Auth(){
  ...
  async function submit(e:FormEvent){
    ...
    try{
      ...
      console.error("YOSSEUF Platform authentication diagnostics",{
        error,
  ...
  return <div className="auth-page"><div className="auth-card"><div className="logo-mark auth-logo"><Brain/></div><span className="section-kicker">PERSONAL OPERATING SYSTEM</span><h1>YOSSEUF Platform</h1><p>Ø³Ø¬Ù‘Ù„ Ø§Ù„Ø¯Ø®ÙˆÙ„ Ù„Ù„ÙˆØµÙˆÙ„ Ø¥Ù„Ù‰ Ù…Ø´Ø§Ø±ÙŠØ¹Ùƒ ÙˆÙ…Ù‡Ø§Ù…Ùƒ.</p><form onSubmit={submit}><label><span>Ø§Ù„Ø¨Ø±ÙŠØ¯ Ø§Ù„Ø¥Ù„ÙƒØªØ±ÙˆÙ†ÙŠ</span><input type="email" value={email} onChange={(e)=>setEmail(e.target.value)} required autoComplete="email" inputMode="email"/></label><button className="primary" disabled={busy}>{busy?"Ø¬Ø§Ø±Ù Ø§Ù„ØªØ­Ù‚Ù‚ ÙˆØ§Ù„Ø¥Ø±Ø³Ø§Ù„â€¦":"Ø¥Ø±Ø³Ø§Ù„ Ø±Ø§Ø¨Ø· Ø§Ù„Ø¯Ø®ÙˆÙ„"}</button></form>{message&&<div className={`auth-message ${messageTone}`} role="status" aria-live="polite">{message}</div>}<small className="auth-diagnostic-note">{APP_INFO.fullLabel}</small></div></div>
}
function LoadingScreen(){return <div className="center-screen"><div className="loader"><Brain size={38}/><span>Ø¬Ø§Ø±Ù ØªØ´ØºÙŠÙ„ YOSSEUF Platformâ€¦</span></div></div>}
