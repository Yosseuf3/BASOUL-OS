...
interface AxisRun { coordinate: number; start: number; end: number }
interface SemanticScale { snap: number; mergeGap: number; crossingTolerance: number; maxRay: number; minSpan: number; maxSpan: number; minArea: number; maxArea: number }

const architecturalSpaceLabel = /room|bed|living|kitchen|bath|toilet|wc|hall|corridor|majlis|office|garage|store|laundry|dining|shop|elevator|shaft|غرفة|نوم|حمام|مطبخ|صالة|صالون|مجلس|ممر|محل|مكتب|مستودع|مصعد|منور|خادمة|ملابس|طعام/i

function scaleFor(document: NormalizedCadDocument): SemanticScale {
  const units = (document.units ?? '').toLowerCase()
  if (units.includes('millimeter') || units === 'mm' || units === '4') return { snap: 20, mergeGap: 2000, crossingTolerance: 150, maxRay: 12000, minSpan: 1200, maxSpan: 12000, minArea: 1_800_000, maxArea: 100_000_000 }
  if (units.includes('centimeter') || units === 'cm' || units === '5') return { snap: 2, mergeGap: 200, crossingTolerance: 15, maxRay: 1200, minSpan: 120, maxSpan: 1200, minArea: 18_000, maxArea: 1_000_000 }
...
function mergeIntervals(intervals: Array<[number, number]>, maxGap: number) {
  const sorted = intervals.map(([a, b]) => a <= b ? [a, b] as [number, number] : [b, a] as [number, number]).sort((a, b) => a[0] - b[0])
...
function buildAxisRuns(document: NormalizedCadDocument, scale: SemanticScale) {
  ...
  for (const entity of document.entities) {
    ...
    for (let i = 0; i + 1 < points.length; i += 1) {
      ...
      const horizontalError = Math.abs(dy) / length, verticalError = Math.abs(dx) / length
      if (verticalError <= Math.sin(2 * Math.PI / 180)) {
        ...
        values.push([a.y, b.y]); vertical.set(coordinate, values)
        ...
        values.push([a.x, b.x]); horizontal.set(coordinate, values)
      }
    }
  }
  const toRuns = (source: Map<number, Array<[number, number]>>): AxisRun[] => {
    const runs: AxisRun[] = []
    for (const [coordinate, intervals] of source) for (const [start, end] of mergeIntervals(intervals, scale.mergeGap)) runs.push({ coordinate, start, end })
    return runs
  }
  return { vertical: toRuns(vertical), horizontal: toRuns(horizontal) }
}

function seedFor(entity: CadEntity): CadPoint | null { return entity.insert ?? entity.points?.[0] ?? null }
function crosses(run: AxisRun, value: number, tolerance: number) { return value >= run.start - tolerance && value <= run.end + tolerance }
...
function nearestDistances(seed: CadPoint, vertical: AxisRun[], horizontal: AxisRun[], tolerance: number) {
  ...
  return { left: left.length ? Math.min(...left) : null, right: right.length ? Math.min(...right) : null, down: down.length ? Math.min(...down) : null, up: up.length ? Math.min(...up) : null }
}
...
export function recoverSemanticRooms(document: NormalizedCadDocument): CadSemanticRoom[] {
  ...
  const labels = document.entities.filter((entity) => {
    ...
    return Boolean(seedFor(entity)) && Boolean(text) && architecturalSpaceLabel.test(text)
  })
  ...
  for (const entity of labels) {
    ...
    const width = d.left + d.right, height = d.down + d.up, area = width * height
    ...
    const boundary: CadPoint[] = [
      { x: seed.x - d.left, y: seed.y - d.down }, { x: seed.x + d.right, y: seed.y - d.down },
      { x: seed.x + d.right, y: seed.y + d.up }, { x: seed.x - d.left, y: seed.y + d.up },
    ]
    rooms.push({ id: `semantic-room:${entity.id}`, labelEntityId: entity.id, label: entity.text!.trim(), seed, boundary, width, height, area, confidence: 0.9 })
  }
  return rooms
}
...
export function applySemanticRoomsToScene(scene: ArchitectureScene, document: NormalizedCadDocument) {
  ...
  for (const room of rooms) {
    scene.nodes[`cad:${room.id}`] = {
      ...
      label: room.label,
      metadata: { source: 'cad-semantic-room-v2.2', labelEntityId: room.labelEntityId, seed: room.seed, width: room.width, height: room.height, area: room.area, confidence: room.confidence },
    }
  }
  scene.metadata = { ...scene.metadata, semanticRoomRecoveryVersion: '2.2', semanticRoomCount: rooms.length }
  return { scene, rooms }
}
