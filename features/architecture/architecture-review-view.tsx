...
import { AlertTriangle, CheckCircle2, FileSearch, FileUp, ShieldCheck, Sparkles } from "lucide-react";
  ...
  return <div className="architecture-review">
    ...
    <section className="architecture-grid">
      <article className="panel drawing-upload-card"><div className="panel-head"><div><span className="section-kicker">01 · الإدراك</span><h2>رفع المخطط</h2></div><FileUp size={22}/></div>
        ...
        {file && <div className="drawing-file-meta"><b>{file.name}</b><span>{formatBytes(file.size)}{pages ? ` · ${pages} صفحة` : ""}</span></div>}{error && <div className="architecture-error"><AlertTriangle size={16}/>{error}</div>}
      </article>
