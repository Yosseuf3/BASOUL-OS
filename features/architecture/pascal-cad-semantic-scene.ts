...
export function buildNativePascalCadScene(document: NormalizedCadDocument, inputGraph?: CadFloorGraph) {
  ...
  for (const edge of graph.edges) {
    ...
    for (const opening of hosted) {
      ...
      if (opening.kind === 'door') { const height = cadOpeningHeight(entity,DOOR_HEIGHT); const cadRotation = cadDoorRotation(entity); const rotationY = cadRotation == null ? 0 : -cadRotation; parsed.push(DoorNode.parse({ id, parentId:wallId, wallId, position:[along,height/2,0], rotation:[0,rotationY,0], width, height })); if (cadRotation != null) cadDoorOrientations.push({ id, entityId:opening.entityId, rotationRadians:cadRotation }); doors += 1 }
      else { const height = cadOpeningHeight(entity,WINDOW_HEIGHT); const sill = cadWindowSill(entity); parsed.push(WindowNode.parse({ id, parentId:wallId, wallId, position:[along,sill+height/2,0], rotation:[0,0,0], width, height })); windows += 1 }
    }
  }
