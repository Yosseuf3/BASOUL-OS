...
export function AdministrationView({ session }: { session: Session }) {
  ...
  return <Panel aria-labelledby="administration-title">
    <span className="section-kicker">YVL ADMINISTRATION</span>
    <h2 id="administration-title">إدارة المؤسسة</h2>
    <p><strong>{organization.name}</strong> · {organization.slug}</p>
    <p>دورك: <Status tone={role === "owner" ? "accent" : "success"}>{role}</Status>{hasOrganizationPermission(role, "organization.update") ? " · إدارة إعدادات المؤسسة متاحة" : " · إعدادات المؤسسة للمالك فقط"}</p>
    ...
    {manage && invitations.length > 0 ? <><h3>الدعوات</h3><TableContainer><table><thead><tr><th>البريد</th><th>الدور</th><th>الحالة</th><th>الانتهاء</th><th>الإجراء</th></tr></thead><tbody>
      {invitations.map((invitation) => <tr key={invitation.id}><td>{invitation.email}</td><td>{invitation.role}</td><td><Status tone={statusTone(invitation.status)}>{invitation.status}</Status></td><td>{new Intl.DateTimeFormat("ar-SA").format(new Date(invitation.expires_at))}</td><td>{invitation.status === "pending" ? <Button type="button" className="ghost" disabled={busy} onClick={() => void run(() => revokeInvitation(organization.id, invitation.id), "أُلغيت الدعوة.")}>إلغاء</Button> : "—"}</td></tr>)}
