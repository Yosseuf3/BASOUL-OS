...
export function DashboardView(props:Props){
 ...
 return <div className="dashboard-v2 executive-dashboard">
  ...
  <section className="dashboard-v2-grid executive-primary">
   ...
   <article className="panel health-card"><div className="health-card-head"><div><span className="section-kicker">Workspace Health 2.0</span><h2>حالة مساحة العمل</h2></div><span className={`health-status health-${health.score>=90?"excellent":health.score>=70?"good":"attention"}`}>{health.label}</span></div><div className="health-main"><div className="health-ring" style={{"--health-score":`${health.score*3.6}deg`} as React.CSSProperties}><div><strong>{health.score}%</strong><small>الصحة العامة</small></div></div><div className="health-insights">{healthNotes.map((note,index)=><span key={note} className={note.includes("متأخرة")||note.includes("متوقفة")||note.includes("معلقة")?"issue":"ok"}>{note.includes("متأخرة")||note.includes("متوقفة")||note.includes("معلقة")?<AlertTriangle size={14}/>:<CheckCircle2 size={14}/>} {note}</span>)}</div></div><button className="health-action" onClick={()=>onNavigate(state.alerts.length?"notifications":"dashboard")}>{state.alerts.length?"راجع نقاط التحسين":"كل الأنظمة مستقرة"}<ChevronLeft size={15}/></button></article>
  </section>
