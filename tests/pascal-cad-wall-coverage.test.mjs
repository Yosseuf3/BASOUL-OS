...
test('Pascal materialization covers every classified CAD wall entity and floor-graph edge', () => {
  ...
  assert.match(source, /const degraded = floatingOpenings > 0 \|\| doors !== expectedDoors \|\| windows !== expectedWindows/)
  assert.match(source, /CAD fidelity gate failed/)
  assert.match(source, /Pascal fidelity gate failed/)
  assert.match(source, /cadFidelityVersion:'3\.3'/)
  assert.match(source, /cadOpeningMaterializationVersion:'3\.3'/)
  assert.match(source, /pascalSemanticIntegrationVersion:'3\.3'/)
})
